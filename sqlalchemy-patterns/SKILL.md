...
description: "SQLAlchemy 2.0, async, ORM patterns — Hermes-native patterns adapted from ECC for Claude Code"
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [hermes, sqlalchemy,patterns]
...
This skill provides Hermes-native patterns for **sqlalchemy-patterns**, adapted from the ECC (Everything Claude Code) ecosystem.

### Quick Reference

| Task | Command |
|------|---------|
| Get started | `hermes -s sqlalchemy-patterns "your task"` |
| With TDD | `hermes -s tdd,sqlalchemy-patterns "build feature with tests"` |
| Security review | `hermes -s security-review,sqlalchemy-patterns "audit implementation"` |

### Common Patterns

```bash
# Example: Load skill and run task
hermes -s sqlalchemy-patterns "Implement user authentication with JWT"

# Multi-skill: Combine with engineering practices
hermes -s sqlalchemy-patterns,agentic-engineering,git-workflow "Build feature end-to-end"
```
