---
name: remix-patterns
description: "Remix v2, loaders, actions, nested routes — Hermes-native patterns adapted from ECC for Claude Code"
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [hermes, remix,patterns]
    related_skills: [hcc-bundle]
---

# remix-patterns

Remix v2, loaders, actions, nested routes
...
This skill provides Hermes-native patterns for **remix-patterns**, adapted from the ECC (Everything Claude Code) ecosystem.

### Quick Reference

| Task | Command |
|------|---------|
| Get started | `hermes -s remix-patterns "your task"` |
| With TDD | `hermes -s tdd,remix-patterns "build feature with tests"` |
| Security review | `hermes -s security-review,remix-patterns "audit implementation"` |

### Common Patterns

```bash
# Example: Load skill and run task
hermes -s remix-patterns "Implement user authentication with JWT"

# Multi-skill: Combine with engineering practices
hermes -s remix-patterns,agentic-engineering,git-workflow "Build feature end-to-end"
```
