...
description: "Strangler fig, feature flags, branches — Hermes-native patterns adapted from ECC for Claude Code"
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [hermes, refactoring,patterns]
    related_skills: [hcc-bundle]
---

# refactoring-patterns

Strangler fig, feature flags, branches
...
This skill provides Hermes-native patterns for **refactoring-patterns**, adapted from the ECC (Everything Claude Code) ecosystem.

### Quick Reference

| Task | Command |
|------|---------|
| Get started | `hermes -s refactoring-patterns "your task"` |
| With TDD | `hermes -s tdd,refactoring-patterns "build feature with tests"` |
| Security review | `hermes -s security-review,refactoring-patterns "audit implementation"` |

### Common Patterns

```bash
# Example: Load skill and run task
hermes -s refactoring-patterns "Implement user authentication with JWT"

# Multi-skill: Combine with engineering practices
hermes -s refactoring-patterns,agentic-engineering,git-workflow "Build feature end-to-end"
```
