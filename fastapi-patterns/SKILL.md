...
description: "FastAPI async, Pydantic v2, dependency injection — Hermes-native patterns adapted from ECC for Claude Code"
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [hermes, fastapi,patterns]
    related_skills: [hcc-bundle]
---

# fastapi-patterns

FastAPI async, Pydantic v2, dependency injection
...
This skill provides Hermes-native patterns for **fastapi-patterns**, adapted from the ECC (Everything Claude Code) ecosystem.

### Quick Reference

| Task | Command |
|------|---------|
| Get started | `hermes -s fastapi-patterns "your task"` |
| With TDD | `hermes -s tdd,fastapi-patterns "build feature with tests"` |
| Security review | `hermes -s security-review,fastapi-patterns "audit implementation"` |

### Common Patterns

```bash
# Example: Load skill and run task
hermes -s fastapi-patterns "Implement user authentication with JWT"

# Multi-skill: Combine with engineering practices
hermes -s fastapi-patterns,agentic-engineering,git-workflow "Build feature end-to-end"
```
