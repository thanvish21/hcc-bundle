...
description: "Go 1.22+, generics, modules, testing, observability — Hermes-native patterns adapted from ECC for Claude Code"
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [hermes, golang,patterns]
    related_skills: [hcc-bundle]
---

# golang-patterns

Go 1.22+, generics, modules, testing, observability
...
This skill provides Hermes-native patterns for **golang-patterns**, adapted from the ECC (Everything Claude Code) ecosystem.

### Quick Reference

| Task | Command |
|------|---------|
| Get started | `hermes -s golang-patterns "your task"` |
| With TDD | `hermes -s tdd,golang-patterns "build feature with tests"` |
| Security review | `hermes -s security-review,golang-patterns "audit implementation"` |

### Common Patterns

```bash
# Example: Load skill and run task
hermes -s golang-patterns "Implement user authentication with JWT"

# Multi-skill: Combine with engineering practices
hermes -s golang-patterns,agentic-engineering,git-workflow "Build feature end-to-end"
```
