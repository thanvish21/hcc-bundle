---
name: mlops
description: "MLflow, Kubeflow, Feast, model registry, drift — Hermes-native patterns adapted from ECC for Claude Code"
version: 1.0.0
author: Hermes Agent
license: MIT
...
MLflow, Kubeflow, Feast, model registry, drift
...
This skill provides Hermes-native patterns for **mlops**, adapted from the ECC (Everything Claude Code) ecosystem.

### Quick Reference

| Task | Command |
|------|---------|
| Get started | `hermes -s mlops "your task"` |
| With TDD | `hermes -s tdd,mlops "build feature with tests"` |
| Security review | `hermes -s security-review,mlops "audit implementation"` |

### Common Patterns

```bash
# Example: Load skill and run task
hermes -s mlops "Implement user authentication with JWT"

# Multi-skill: Combine with engineering practices
hermes -s mlops,agentic-engineering,git-workflow "Build feature end-to-end"
```
