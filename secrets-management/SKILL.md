...
description: "Vault, SOPS, SealedSecrets, Doppler — Hermes-native patterns adapted from ECC for Claude Code"
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [hermes, secrets,management]
    related_skills: [hcc-bundle]
---

# secrets-management

Vault, SOPS, SealedSecrets, Doppler
...
This skill provides Hermes-native patterns for **secrets-management**, adapted from the ECC (Everything Claude Code) ecosystem.

### Quick Reference

| Task | Command |
|------|---------|
| Get started | `hermes -s secrets-management "your task"` |
| With TDD | `hermes -s tdd,secrets-management "build feature with tests"` |
| Security review | `hermes -s security-review,secrets-management "audit implementation"` |

### Common Patterns

```bash
# Example: Load skill and run task
hermes -s secrets-management "Implement user authentication with JWT"

# Multi-skill: Combine with engineering practices
hermes -s secrets-management,agentic-engineering,git-workflow "Build feature end-to-end"
```
