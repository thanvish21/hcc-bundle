---
name: astro-patterns
description: "Astro islands, partial hydration, content collections — Hermes-native patterns adapted from ECC for Claude Code"
version: 1.0.0
author: Hermes Agent
license: MIT
...
This skill provides Hermes-native patterns for **astro-patterns**, adapted from the ECC (Everything Claude Code) ecosystem.

### Quick Reference

| Task | Command |
|------|---------|
| Get started | `hermes -s astro-patterns "your task"` |
| With TDD | `hermes -s tdd,astro-patterns "build feature with tests"` |
| Security review | `hermes -s security-review,astro-patterns "audit implementation"` |

### Common Patterns

```bash
# Example: Load skill and run task
hermes -s astro-patterns "Implement user authentication with JWT"

# Multi-skill: Combine with engineering practices
hermes -s astro-patterns,agentic-engineering,git-workflow "Build feature end-to-end"
```
