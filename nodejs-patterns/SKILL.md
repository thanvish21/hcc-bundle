...
description: "Node 20+, ESM, worker_threads, diagnostics — Hermes-native patterns adapted from ECC for Claude Code"
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [hermes, nodejs,patterns]
    related_skills: [hcc-bundle]
---

# nodejs-patterns

Node 20+, ESM, worker_threads, diagnostics
...
This skill provides Hermes-native patterns for **nodejs-patterns**, adapted from the ECC (Everything Claude Code) ecosystem.

### Quick Reference

| Task | Command |
|------|---------|
| Get started | `hermes -s nodejs-patterns "your task"` |
| With TDD | `hermes -s tdd,nodejs-patterns "build feature with tests"` |
| Security review | `hermes -s security-review,nodejs-patterns "audit implementation"` |

### Common Patterns

```bash
# Example: Load skill and run task
hermes -s nodejs-patterns "Implement user authentication with JWT"

# Multi-skill: Combine with engineering practices
hermes -s nodejs-patterns,agentic-engineering,git-workflow "Build feature end-to-end"
```
