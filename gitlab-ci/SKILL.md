---
name: gitlab-ci
description: "Pipelines, rules, needs, DAG, environments — Hermes-native patterns adapted from ECC for Claude Code"
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [hermes, gitlab,ci]
    related_skills: [hcc-bundle]
---

# gitlab-ci

Pipelines, rules, needs, DAG, environments
...
This skill provides Hermes-native patterns for **gitlab-ci**, adapted from the ECC (Everything Claude Code) ecosystem.

### Quick Reference

| Task | Command |
|------|---------|
| Get started | `hermes -s gitlab-ci "your task"` |
| With TDD | `hermes -s tdd,gitlab-ci "build feature with tests"` |
| Security review | `hermes -s security-review,gitlab-ci "audit implementation"` |

### Common Patterns

```bash
# Example: Load skill and run task
hermes -s gitlab-ci "Implement user authentication with JWT"

# Multi-skill: Combine with engineering practices
hermes -s gitlab-ci,agentic-engineering,git-workflow "Build feature end-to-end"
```
