...
description: "Prisma ORM, middleware, transactions, migrations — Hermes-native patterns adapted from ECC for Claude Code"
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [hermes, prisma,patterns]
    related_skills: [hcc-bundle]
---

# prisma-patterns

Prisma ORM, middleware, transactions, migrations
...
This skill provides Hermes-native patterns for **prisma-patterns**, adapted from the ECC (Everything Claude Code) ecosystem.

### Quick Reference

| Task | Command |
|------|---------|
| Get started | `hermes -s prisma-patterns "your task"` |
| With TDD | `hermes -s tdd,prisma-patterns "build feature with tests"` |
| Security review | `hermes -s security-review,prisma-patterns "audit implementation"` |

### Common Patterns

```bash
# Example: Load skill and run task
hermes -s prisma-patterns "Implement user authentication with JWT"

# Multi-skill: Combine with engineering practices
hermes -s prisma-patterns,agentic-engineering,git-workflow "Build feature end-to-end"
```
