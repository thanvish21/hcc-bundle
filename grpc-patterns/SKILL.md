---
name: grpc-patterns
description: "gRPC, protobuf, interceptors, streaming — Hermes-native patterns adapted from ECC for Claude Code"
version: 1.0.0
author: Hermes Agent
license: MIT
...
This skill provides Hermes-native patterns for **grpc-patterns**, adapted from the ECC (Everything Claude Code) ecosystem.

### Quick Reference

| Task | Command |
|------|---------|
| Get started | `hermes -s grpc-patterns "your task"` |
| With TDD | `hermes -s tdd,grpc-patterns "build feature with tests"` |
| Security review | `hermes -s security-review,grpc-patterns "audit implementation"` |

### Common Patterns

```bash
# Example: Load skill and run task
hermes -s grpc-patterns "Implement user authentication with JWT"

# Multi-skill: Combine with engineering practices
hermes -s grpc-patterns,agentic-engineering,git-workflow "Build feature end-to-end"
```
