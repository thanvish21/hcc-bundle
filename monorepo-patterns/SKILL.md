...
description: "Turborepo, Nx, workspace, caching — Hermes-native patterns adapted from ECC for Claude Code"
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [hermes, monorepo,patterns]
    related_skills: [hcc-bundle]
---

# monorepo-patterns

Turborepo, Nx, workspace, caching
...
This skill provides Hermes-native patterns for **monorepo-patterns**, adapted from the ECC (Everything Claude Code) ecosystem.

### Quick Reference

| Task | Command |
|------|---------|
| Get started | `hermes -s monorepo-patterns "your task"` |
| With TDD | `hermes -s tdd,monorepo-patterns "build feature with tests"` |
| Security review | `hermes -s security-review,monorepo-patterns "audit implementation"` |

### Common Patterns

```bash
# Example: Load skill and run task
hermes -s monorepo-patterns "Implement user authentication with JWT"

# Multi-skill: Combine with engineering practices
hermes -s monorepo-patterns,agentic-engineering,git-workflow "Build feature end-to-end"
```
