...
description: "Modules, workspaces, state, providers — Hermes-native patterns adapted from ECC for Claude Code"
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [hermes, terraform,patterns]
    related_skills: [hcc-bundle]
---

# terraform-patterns

Modules, workspaces, state, providers
...
This skill provides Hermes-native patterns for **terraform-patterns**, adapted from the ECC (Everything Claude Code) ecosystem.

### Quick Reference

| Task | Command |
|------|---------|
| Get started | `hermes -s terraform-patterns "your task"` |
| With TDD | `hermes -s tdd,terraform-patterns "build feature with tests"` |
| Security review | `hermes -s security-review,terraform-patterns "audit implementation"` |

### Common Patterns

```bash
# Example: Load skill and run task
hermes -s terraform-patterns "Implement user authentication with JWT"

# Multi-skill: Combine with engineering practices
hermes -s terraform-patterns,agentic-engineering,git-workflow "Build feature end-to-end"
```
