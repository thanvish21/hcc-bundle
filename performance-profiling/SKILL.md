...
description: "pprof, py-spy, perf, flamegraphs — Hermes-native patterns adapted from ECC for Claude Code"
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [hermes, performance,profiling]
    related_skills: [hcc-bundle]
---

# performance-profiling

pprof, py-spy, perf, flamegraphs
...
This skill provides Hermes-native patterns for **performance-profiling**, adapted from the ECC (Everything Claude Code) ecosystem.

### Quick Reference

| Task | Command |
|------|---------|
| Get started | `hermes -s performance-profiling "your task"` |
| With TDD | `hermes -s tdd,performance-profiling "build feature with tests"` |
| Security review | `hermes -s security-review,performance-profiling "audit implementation"` |

### Common Patterns

```bash
# Example: Load skill and run task
hermes -s performance-profiling "Implement user authentication with JWT"

# Multi-skill: Combine with engineering practices
hermes -s performance-profiling,agentic-engineering,git-workflow "Build feature end-to-end"
```
