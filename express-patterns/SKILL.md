...
description: "Express 5, middleware, validation, error handling — Hermes-native patterns adapted from ECC for Claude Code"
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [hermes, express,patterns]
    related_skills: [hcc-bundle]
---

# express-patterns

Express 5, middleware, validation, error handling
...
This skill provides Hermes-native patterns for **express-patterns**, adapted from the ECC (Everything Claude Code) ecosystem.

### Quick Reference

| Task | Command |
|------|---------|
| Get started | `hermes -s express-patterns "your task"` |
| With TDD | `hermes -s tdd,express-patterns "build feature with tests"` |
| Security review | `hermes -s security-review,express-patterns "audit implementation"` |

### Common Patterns

```bash
# Example: Load skill and run task
hermes -s express-patterns "Implement user authentication with JWT"

# Multi-skill: Combine with engineering practices
hermes -s express-patterns,agentic-engineering,git-workflow "Build feature end-to-end"
```
