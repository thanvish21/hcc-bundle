---
name: redis-patterns
description: "Redis 7, streams, Lua scripts, clustering — Hermes-native patterns adapted from ECC for Claude Code"
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [hermes, redis,patterns]
    related_skills: [hcc-bundle]
---

# redis-patterns

Redis 7, streams, Lua scripts, clustering
...
This skill provides Hermes-native patterns for **redis-patterns**, adapted from the ECC (Everything Claude Code) ecosystem.

### Quick Reference

| Task | Command |
|------|---------|
| Get started | `hermes -s redis-patterns "your task"` |
| With TDD | `hermes -s tdd,redis-patterns "build feature with tests"` |
| Security review | `hermes -s security-review,redis-patterns "audit implementation"` |

### Common Patterns

```bash
# Example: Load skill and run task
hermes -s redis-patterns "Implement user authentication with JWT"

# Multi-skill: Combine with engineering practices
hermes -s redis-patterns,agentic-engineering,git-workflow "Build feature end-to-end"
```
